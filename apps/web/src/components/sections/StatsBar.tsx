...
export function StatsBar() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative pt-64 pb-12 md:pb-20 bg-[var(--bg)] border-t-[2px] border-[var(--bg)] overflow-hidden transition-colors duration-500 z-20"
    >
      ...
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex items-center justify-center opacity-[0.03] select-none pointer-events-none">
        <span className="text-[180px] md:text-[240px] font-display font-black text-[var(--text)] italic uppercase tracking-tighter whitespace-nowrap rotate-[-5deg]">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        {/* World-Class Header (Rescaled) */}
        <div className="flex flex-col items-center text-center mb-12 relative">
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={isInView ? { opacity: 1, scale: 1 } : {}}
            className="mb-6 bg-[var(--text)] text-[var(--bg)] px-6 py-2 font-mono font-black text-xs uppercase italic rotate-2 rounded-xl shadow-xl border-4 border-[var(--text)]"
          >
            GLOBAL METRICS
          </motion.div>
          <motion.h2
            ...
            className="text-[48px] md:text-[80px] font-display font-black flex flex-col leading-[0.85] tracking-[-0.04em] italic uppercase"
          >
...
function StatCard({ stat, index, isInView }: { stat: any, index: number, isInView: boolean }) {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.9, y: 60, rotateX: 45 }}
      animate={isInView ? {
        opacity: 1,
        scale: 1,
        y: 0,
      ...
      whileHover={{ y: -6, rotateZ: 0, scale: 1.04, transition: { duration: 0.4 } }}
      className="relative group h-[340px] bg-[var(--bg-card)] border-[4px] border-[var(--text)] p-7 rounded-[48px] flex flex-col justify-between overflow-hidden shadow-[12px_12px_0_var(--purple)]/5 hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all duration-700"
    >
      <div
      ...
      <div className="relative z-10 flex flex-col h-full justify-between items-center text-center pt-8">
        <div className="space-y-4">
          <div className="flex items-baseline justify-center gap-1 group-hover:scale-105 transition-transform duration-700">
            <span className="text-[54px] md:text-[64px] font-display font-black text-[var(--text)] italic leading-none tracking-[-0.06em]">
              <CountUp value={stat.value} isInView={isInView} delay={index * 0.2 + 0.8} />
            </span>
            <span className="text-2xl font-display font-black italic leading-none" style={{ color: stat.color }}>{stat.suffix}</span>
          </div>
