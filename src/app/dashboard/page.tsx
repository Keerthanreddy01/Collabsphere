            ...
            <div className="flex-1 space-y-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button className="text-[12px] text-[#58a6ff] hover:underline">Customize your pins →</button>
                    </div>
                        ...
                        {pinnedProjects.map((p, i) => (
                            <div key={i} className="bg-[#161b22] border border-[#30363d] rounded-[6px] p-4 group hover:bg-[#1c2128] transition-colors">
                                <div className="flex items-start justify-between mb-2">
                                    ...
                                    <span className={cn(
                                        "text-[12px] px-2 py-0.5 rounded-full border font-medium",
                                        p.status === "Building" && "text-[#3fb950] border-[#3fb950] bg-[#1f2d1f]/30",
                                        p.status === "Needs Help" && "text-[#f85149] border-[#f85149] bg-[#2d1f1f]/30",
                                    )}>
                                        {p.status}
                                    </span>
                                </div>
                ...
                <div className="pt-4">
                    ...
                    <div className="surface p-0 divide-y divide-[#30363d]">
                        {[
                            { user: "Sarah Chen", project: "Nexus Chat", time: "5m ago", text: "Integrated the new libp2p pubsub mechanism for better message propagation." },
                            { user: "Marcus Vane", project: "DecentralShop", time: "2h ago", text: "Added support for multi-sig vault contracts in the checkout flow." }
                        ].map((activity, i) => (
                            <div key={i} className="p-4 hover:bg-[#1c2128] transition-colors">
                                <div className="flex items-center gap-2 text-[13px] mb-1">
                                    <Avatar className="h-5 w-5 rounded-full">
                                        <AvatarImage src={`https://avatar.vercel.sh/${activity.user}`} />
                                    </Avatar>
                                    <span className="font-semibold text-[#e6edf3]">{activity.user}</span>
                                    <span className="text-[#7d8590]">posted an update to</span>
                                    <span className="text-[#58a6ff] hover:underline cursor-pointer">{activity.project}</span>
                                    <span className="text-[#7d8590]">· {activity.time}</span>
                                </div>
                                <p className="text-[13px] text-[#7d8590] truncate pl-7">
                                    {activity.text}
                                </p>
                            </div>
                        ))}
